...
const FilterForm = ({
  ...
  const { control, reset, handleSubmit } = useForm<IFiltersForm>({
    defaultValues: { ...(searchParams as IFiltersForm) },
  });
      ...
      <Flex vertical className="min-w-[min(95dvh,300px)]">
        ...
        <Flex vertical gap={4} className="p-4 w-full">
          <Flex vertical gap={4} className="w-full">
            {types
              ?.filter((e) => e in FilterMap && !!FilterMap?.[e]?.connected_key)
              ?.map((key) => (
                <Flex vertical gap={2} key={key} className="w-full">
                  ...
                  <Controller
                    name={FilterMap?.[key]?.connected_key as any}
                    control={control}
...
> = {
  ...
  [SubjectFilters.TrainingTypes]: {
    name: "Mashg'ulot",
    connected_key: FilterItem.TrainingType,
    sortFn: () => 0,
    placeholder: "Mashg'ulotni tanlang",
    render: (obj: ITrainingType) => ({
      label: obj?.name,
      value: obj?.code,
    }),
  },
